		...
		{	
			...
			if(jacobians != NULL)
			{
				if(jacobians[0] != NULL)
				{
					...
					Eigen::Matrix<double, 1, 3> prefix (nu.x()/nu.norm(),nu.y()/nu.norm(), nu.z()/nu.norm());
					//skewe_re : skew of (p_b - p_a)
					//dp_by_so3 : with translation and rotation
					std::cout <<"prefix "<<prefix<<std::endl;

					J_se3.block<1,6>(0,0) = prefix * skew_re * dp_by_so3/de.norm();
...
struct LidarEdgeFactor
{
	...
	/*
	Reference:
	In most optimization problems small groups of scalars occur together. For example the 
	three components of a translation vector and the four components of the quaternion that define the pose of a camera.
    ...
    = new AutoDiffCostFunction<MyScalarCostFunctor, 1, 2, 2>(
        new MyScalarCostFunctor(1.0));              ^  ^  ^
                        ...
                        Dimension of x ----------------+  |
                        Dimension of y -------------------+
	<1, 2, 2> describe the functor as computing a 1-dimensional output from two arguments, both 2-dimensional.
	*/

	static ceres::CostFunction *Create(const Eigen::Vector3d curr_point_, const Eigen::Vector3d last_point_a_,
									   const Eigen::Vector3d last_point_b_, const double s_)
	{
		//set up only cost function (also known as residual). This uses 
