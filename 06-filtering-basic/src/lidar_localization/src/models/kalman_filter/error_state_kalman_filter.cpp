...
void ErrorStateKalmanFilter::Init(const Eigen::Vector3d &vel,
  ...
  angular_vel_init = angular_vel_init - gyro_bias_ - C_nb.transpose() * w_;//TODO: check
...
void ErrorStateKalmanFilter::SetProcessEquation(const Eigen::Matrix3d &C_nb,
  ...
  //F_.block<3, 3>(INDEX_ERROR_VEL, INDEX_ERROR_ACCEL) = -C_nb;  // b. set process equation for delta ori:
  F_.block<3, 3>(INDEX_ERROR_ORI, INDEX_ERROR_ORI) =
...
void ErrorStateKalmanFilter::CorrectErrorEstimationPose(
  ...
  K = P_ * G.transpose() *(G * P_ * (G.transpose()) + C * R * C).inverse(); 
          // assuming C.transpose =C, because c is identity matrix
...
void ErrorStateKalmanFilter::CorrectErrorEstimation(
  ...
  //
  MatrixP Iden_mat = Eigen::MatrixXd::Identity(DIM_STATE, DIM_STATE);
  P_ = (Iden_mat - K * G) * P_;
  X_ = X_ + K * (Y - G * X_);
}
...
void ErrorStateKalmanFilter::EliminateError(void) {
  ...
  Eigen::Matrix3d C_nn = // exponential map
  ...
  //          << std::endl;
  // d. gyro bias:
  if (0&&IsCovStable(INDEX_ERROR_GYRO)) {
  ...
  // e. accel bias:
  if (0&&IsCovStable(INDEX_ERROR_ACCEL)) {
